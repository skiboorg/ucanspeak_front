...
<template>
  <BlockCourseHeader title="Курс для взрослых" :count="course?.levels.length" show_profile/>
    <div class="space-y-4">
  <NuxtLink v-for="level in course.levels"  :to="`/courses/${course.slug}/${level.slug}`" class="flex items-start gap-4 p-5 bg-white border border-[#18181b]/10 rounded-[20px]">
    <div class="shrink-0 w-16 h-16 flex items-center justify-center rounded-[10px] bg-[#ffe5f7] border border-[18181b]/[0.5]">
      <img src="~/assets/images/air-card.svg" alt="" class="mix-blend-luminosity">
    </div>
    <div class="w-full flex flex-col gap-[3px]">
      ...
      <p class="font-normal text-xs leading-[130%] text-[#8f8fa3]">{{level.description}} 42 урока</p>

      <UIPLine :value="30" class="my-[5px]"/>
      <p class="font-normal text-xs leading-[130%] text-[#8f8fa3]">2 из 37 уроков пройдено</p>
    </div>
  </NuxtLink>
  </div>
</template>
